...
#ifdef   TURBO    /* is this a good define? */
void          UpdateCRC( s, len)
unsigned char *s;
register int  len;
 /* update running CRC calculation with contents of a buffer */
...
/*
 * 65816 assembly version of 32 bit CRC calculation.
 * Written by Martin E. Peckham. Given to the public domain.
 *
 */
#ifdef   APW
void     UpdateCRC( s, len)
 ...
 /* update running CRC calculation with contents of a buffer */
{
    ...
    loop:
         ...
         lda  crc32val+1      /* crc >> 8 */
         sta  crc32val
         lda  crc32val+3
         and  #0xFF           /* crc & 0x00FFFFFF */
         sta  crc32val+2
    ...
    local0:
         eor  tmp             /* *s ^ (byte )res */
         asl  a               /* times 4 for a 'long' */
         asl  a
         tax
         lda  >crc_32_tab,x   /* tab[*s ^ (byte )res] */
         eor  crc32val
         sta  crc32val
         lda  >crc_32_tab+2,x
         eor  crc32val+2
