 ...
 /* expand probablisticly reduced data */
{
    ...
    while (((outpos+outcnt) < lrec.uncompressed_size) && (!zipeof)) {
         ...
         lchar = nchar;       /* store character for next iteration */
    }
}
...
void unShrink()
{
    ...
    while (!zipeof) {
         ...
         code = free_ent;     /* generate new entry */
         if (code < maxcodemax) {
    ...
    do { 
         do { 
              ...
              { 
                  ...
                  if ((a > b) || ((a == b)
                      && (tree->entry[x].Value > tree->entry[x + gap].Value)))
                  { 
                      t                    = tree->entry[x]; 
...
void     GenerateTrees( tree)     /* Generate the Shannon-Fano trees */
sf_tree  *tree;
{
    word      Code;
    unsigned  CodeIncrement;
    unsigned  LastBitLength;
    int       i;
...
void     unImplode()          /* expand imploded data */ 
{ 
    int       lout;
    longint   op;
    int       Length;
    ...
    { 
         ...
         {                
              ...
              /* normal copy of data from output buffer */
              {
                  ...
                  /* do a block memory copy if possible */
                  if( (ix + Length < OUTBUFSIZ) && (outcnt+Length < OUTBUFSIZ) )
                  {
                      memcpy( outptr, &outbuf[ix], Length );
                      outptr += Length;
                      outcnt += Length;
                  }
                  else                    /* otherwise copy byte by byte */
                      while (Length--)
                      {
                          OUTB(outbuf[ix]);
                          if (++ix >= OUTBUFSIZ)
...
void extract_member()
{
    ...
    crc32val = -1 - crc32val;
    if (crc32val != lrec.crc32)   /* are the following CRC values reversed? */
...
{
    read(zipfd, (char *)&lrec, sizeof(lrec)); /* MEP added cast */
...
void process_central_file_header()
{
    ...
    read(zipfd, (char *)&rec, sizeof(rec));   /* MEP added cast */
...
void process_headers()
{
    ...
    while (1) {       /* MEP added cast in read() */
