 ...
 *
 *  ported to VAX by Kw Chiu and Francis Leung
 *
 * To compile using Turbo C:
 *      tcc -B -O -Z -G -mc unzip.c crc32.c    ;turbo C 2.0, compact model
 *
 * To compile in VAX:
 *      cc unzip,crc32
 *      link unzip,crc32
...
typedef unsigned short word;  /* short in VAX */
...
typedef struct end_central_dir_record {
	...
	word number_disk_with_start_central;
	word total_entries_central_dir_on_th;
...
{
#ifdef VAX
    /* don't know how to set file time */
#else
 ...
 /* flush contents of output buffer */
{
	...
	outcnt = 0;
	outptr = outbuf;
    printf (".");       /* inform user of progress. - Francis Leung */
}
...
#ifndef VAX
                  /* it appeared that the VAX memcpy function is not
                   * working the same way as Turbo C, therefore use
                   * OUTB instead.  - Francis Leung
                   */
...
#ifndef VAX     /* added by Francis Leung */
...
{ 
   ...
   do { 
      do { 
         ...
         { 
            a = tree->entri[x].BitLength; 
            b = tree->entri[x + gap].BitLength; 
            if ((a > b) || ((a == b) && (tree->entri[x].Value > tree->entri[x + gap].Value))) 
            { 
               t = tree->entri[x]; 
               tree->entri[x] = tree->entri[x + gap]; 
               tree->entri[x + gap] = t; 
               noswaps = 0;
            } 
         } 
   ...
   while (treeBytes > 0)
   {
      ...
      while (num > 0)
      {
         if (len > tree->MaxLength)
            tree->MaxLength = len;
         tree->entri[i].BitLength = len;
         tree->entri[i].Value = i;
         i++;
         num--;
      }
...
{ 
   ...
   { 
      Code += CodeIncrement; 
      if (tree->entri[i].BitLength != LastBitLength) 
      { 
         LastBitLength = tree->entri[i].BitLength; 
         CodeIncrement = 1 << (16 - LastBitLength); 
      } 

      tree->entri[i].Code = Code; 
      i--; 
   } 
} 
   ...
   *dest = -1;   /* in case of error */ 
   ...
   { 
      ...
      */
      ...
      while (tree->entri[cur].BitLength < bits) 
      { 
         cur++; 
         if (cur >= tree->entries) 
            return; /* data error */
      ...
      while (tree->entri[cur].BitLength == bits) 
      { 
         if (tree->entri[cur].Code == cv) 
         { 
            *dest = tree->entri[cur].Value; 
            return; 
         } 
...
#ifdef VAX
   printf("Modified by K.W.Chiu and Francis Leung to work on VAX.\n");
   printf("   Kw Chiu, Sysop of Chiu's Board (IFNA 700/138), Hong Kong.\n");
   printf("   ..!uunet!hkucs!hkucc!hcxcckw\n");
   printf("File date and time not restored. Please teach me how to do so.\n");
#endif
