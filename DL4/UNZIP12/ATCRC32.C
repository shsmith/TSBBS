 ...
 /* update running CRC calculation with contents of a buffer */
{
	   ...
	   while (len--)
	       {
                a=crc_32_tab[(byte)a^(byte)*s++] ^ ((a>>8) & 0x00FFFFFFL);
           }
...
{
UpdateCRC:
	move.l	4(A7), A1					;address of data to compute crc on
	move.w	8 (A7), D3					;length of data
	move.l	crc32val, D2				;global crc value

	lea.l	crc_32_tab,A0				;address of crc table
	subq.w 	#1,D3						;subtract 1 for dbf loop
top_loop:
	clr.w	D0
	move.b	D2,D0						;lower byte of global crc
	clr.w	D1
	move.b	(A1)+,D1					;first byte of data
	eor.w	D1,D0						;exclusive or with crc
	add.w	D0,D0						;2 * result
	add.w	D0,D0						;4 * result (for long index)
	move.l	0(A0,D0.w),D1				;crc value from table
	clr.b	D2							;clear lower byte
	ror.l	#8,D2						;32 bit -> 24 bit (upper byte clear)
	eor.l	D1,D2						;exclusive or with global crc
	dbf		D3,top_loop					;loop until end of data

	move.l	D2, crc32val				;update global crc value
	rts
}
